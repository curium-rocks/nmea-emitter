pipeline {
  ...
  stages {
    stage('Notify') {
      steps {
        mattermostSend color: "warning", message: "[NMEA-Emitter] Build Started - ${env.JOB_NAME} ${env.BUILD_NUMBER} (<${env.BUILD_URL}|Open>)"
      }
    }
    //always run
    stage('Build') {
  ...
  post {
    failure {
       mattermostSend color: "danger", message: "[NMEA-Emitter] Build Failure - ${env.JOB_NAME} ${env.BUILD_NUMBER} (<${env.BUILD_URL}|Open>)"
    }
    success{
       mattermostSend color: "good", message: "[NMEA-Emitter] Build Success - ${env.JOB_NAME} ${env.BUILD_NUMBER} (<${env.BUILD_URL}|Open>)"
    }
    always {
